...
eclipse.resetEverything = function() {
  eclipse.confirm('Are you sure you want to reset Eclipse?', 'Eclipse',
    function() {
    ...
    function() {
      eclipse.alert('eclipse is unaffected.', 'eclipse');
    }
  );
}
...
eclipse.ret_san = function(str) {
  return str.replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\'/g, "");
}

// Exports localStorage as JSON

eclipse.exportLocalStorage = function(downloadLocalStorage) {
  var mem;
  var storage = "";
...
eclipse.recover = function(text) {
  ...
  try {
    ...
    for (var i = 0; i < JSON.parse(text).eclipse.length; i++) {
      if (ret_san(JSON.stringify(JSON.parse(text).eclipse[i].name)).replace(/\"/g, "") == "currentSkin") {
...
eclipse.load_eclipse_storage_file = function(file, url) {
  eclipse.confirm('Performing this action will erase all of your data and overwrite it with the data contained in the file, are you sure you would like to continue?', 'eclipse',
    function() {
      eclipse.showPreloader('Loading');
      window.setTimeout(function() {
        if (file && url) {
          eclipse.hidePreloader();
          eclipse.alert('You cannot input a file and a URL.', 'eclipse');
          return false;
        }
        if (file) {
          if (document.getElementById("localFile").files[0] == null) {
            eclipse.hidePreloader();
            eclipse.alert("You must specify a file.", 'eclipse');
            return false;
          }
          ...
          if (f) {
            ...
            r.onload = function(e) {
              ...
              if (contents == "") {
                eclipse.hidePreloader();
                eclipse.alert("An error has occurred and the operation was cancelled.", 'eclipse');
                return false;
              }
              window.setTimeout(function() {
                ...
                try {
                  ...
                  for (var i = 0; i < JSON.parse(contents).eclipse.length; i++) {
                    if (ret_san(JSON.stringify(JSON.parse(contents).eclipse[i].name)).replace(/\"/g, "") == "currentSkin") {
                  ...
                  eclipse.hidePreloader();
                  eclipse.showPreloader('Recovering');
                  window.setTimeout(function() {
                    try {
                      recover(localStorageJSON);
                    } catch (err) {
                      eclipse.alert("An critical error has occurred and eclipse was reset.", 'eclipse', function() {
                        location.reload()
                      });
                      return false;
                    }
            ...
            eclipse.alert("Failed to read file.", "eclipse");
          }
        }
        if (url) {
          if (!url.toLowerCase().match(/\.(eclipse)$/)) {
            eclipse.hidePreloader();
            eclipse.alert('This function can only load files with the extension ".eclipse".', 'eclipse');
            return false;
          }
        ...
        if (!file && !url) {
          eclipse.hidePreloader();
          eclipse.alert('Requires input', 'eclipse');
          return false;
        }
      }, 1);
    },
